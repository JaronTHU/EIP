    ...
    def __call__(self, data):
        if hasattr(data, 'keys'):
            if self.append_xyz:
                ...
                data['heights'] = data['pos'] - torch.min(data['pos'], dim=0, keepdim=True)[0]
            else:
...
@DataTransforms.register_module()
class PointCloudRotationV2(object):
    """My random rotation, generate 6D vector for rotations
    ...
    def __call__(self, data):
        ...
        rv = torch.randn(2, 3, device=device)
        r1 = normalize(rv[0], dim=0)
        r2 = normalize(torch.cross(r1, rv[1]), dim=0)
        r3 = torch.cross(r1, r2)
        rot_mat = torch.stack([r1, r2, r3], dim=0)

        if hasattr(data, 'keys'):
            data['pos'] = data['pos'] @ rot_mat
            if 'normals' in data:
                data['normals'] = data['normals'] @ rot_mat
        else:
            data = data @ rot_mat
        return data
