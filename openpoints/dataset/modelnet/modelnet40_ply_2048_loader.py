...
class ModelNet40Ply2048(Dataset):
    ...
    def __init__(self,
        ...
        self.data, self.normal, self.label = load_data(data_dir, self.partition, self.url)
    ...
    def __getitem__(self, item):
        ...
        normal = self.normal[item][:self.num_points]
        label = self.label[item]

        if self.partition == 'train':
            pointcloud, normal = shuffle(pointcloud, normal)
        data = {'pos': pointcloud,
                'normals': normal,
                'y': label}
