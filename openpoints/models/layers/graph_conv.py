...
class EdgeConv(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        max_value, _ = torch.max(self.nn(torch.cat([x.expand(-1, -1, -1, edge_index.shape[-1]), x_j - x], dim=1)), -1, keepdim=True)
        return max_value
...
class EdgeConvS(nn.Module):
    ...
    Edge convolution layer (with activation, batch normalization) for dense data type
    """
    def __init__(self, in_channels, out_channels, hidden_channels=[], **kwargs):
        super().__init__()
        hidden_channels = [2 * in_channels] + hidden_channels + [out_channels]

        self.nn = []
        for i in range(len(hidden_channels) - 1):
            self.nn.extend(list(create_convblock2d(hidden_channels[i], hidden_channels[i+1], **kwargs)))
