...
from openpoints.transforms import build_transforms_from_cfg, tetrahedral, octahedral, icosahedral, random_rot
...
def main(gpu, cfg, profile=False):
    ...
    if hasattr(model, 'pose_fn') and hasattr(model.pose_fn, 'reg') and hasattr(model.pose_fn.reg, 'update_cf'):
        model.pose_fn.reg.update_cf()
    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == 'resume':
            ...
            if cfg.mode == 'test':
                ...
                return True
            elif cfg.mode == 'test_so3':
                # test mode
                epoch, best_val = load_checkpoint(
                    model, pretrained_path=cfg.pretrained_path)
                macc, oa, accs, cm = validate_so3(model, test_loader, cfg)
                print_cls_results(oa, macc, accs, epoch, cfg)
                return True
            elif cfg.mode == 'val':
                # validation mode
    ...
    # only destroy process group when distributed
    if cfg.distributed:
...
def train_one_epoch(model, train_loader, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        # WARNING! Test get_logits_loss
        # if pose_num_iter == cfg.pose_step_per_update:
        #     pose_num_iter = 0
        #     logits, loss = model.get_logits_loss(data, target, update_pose=True)
        # else:
        #     logits, loss = model.get_logits_loss(data, target, update_pose=False)
        ...
        target = target.unsqueeze(-1).repeat(1, logits.size(0) // data['pos'].size(0)).view(-1)

        cm.update(logits.max(dim=1)[1], target)
...
@torch.no_grad()
def validate_so3(model, val_loader, cfg):
    model.eval()  # set model to eval mode
    cm = ConfusionMatrix(num_classes=cfg.num_classes)
    npoints = cfg.num_points
    pbar = tqdm(enumerate(val_loader), total=val_loader.__len__())
    for idx, data in pbar:
        for key in data.keys():
            data[key] = data[key].cuda(non_blocking=True)
        target = data['y']
        # default cfg.model.in_channels=3
        new_points = data['x'][:, :npoints, :3].clone()
        if cfg.so3_mode == 'random_rot':
            for _ in range(cfg.num_rot):
                r = random_rot(new_points.size(0)).to(new_points.device)
                data['pos'] = new_points @ r
                data['x'] = data['pos'].transpose(1, 2).contiguous()
                if 'normals' in data:
                    data['normals'] = data['normals'] @ r
                logits = model(data)
                cm.update(logits.argmax(dim=1), target)
        elif cfg.so3_mode == 'finite_rot':
            if cfg.rot_group == 'tet':
                r = tetrahedral()
            elif cfg.rot_group == 'oct':
                r = octahedral()
            elif cfg.rot_group == 'ico':
                r = icosahedral()
            else:
                raise ValueError()
            r = r.to(new_points.device)
            for i in range(r.size(0)):
                data['pos'] = new_points @ r[i].unsqueeze(0)
                data['x'] = data['pos'].transpose(1, 2).contiguous()
                if 'normals' in data:
                    data['normals'] = data['normals'] @ r
                logits = model(data)
