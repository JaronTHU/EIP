    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == 'resume':
            ...
            if cfg.mode in ['val', 'test']:
                ...
                # test mode
                load_checkpoint(model, pretrained_path=cfg.pretrained_path)
                test_ins_miou, test_cls_miou, test_cls_mious = validate_so3(model, val_loader, cfg, num_votes=cfg.num_votes, data_transform=voting_transform)
                return test_ins_miou
...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        #     target = target.unsqueeze(1).repeat(1, logits.size(0) // target.size(0)).flatten(0, 1)
        #     data['cls'] = data['cls'].unsqueeze(1).repeat(1, logits.size(0) // data['cls'].size(0)).flatten(0, 1)
...
@torch.no_grad()
def validate_so3(model, val_loader, cfg, num_votes=0, data_transform=None):
    model.eval()  # set model to eval mode
    pbar = tqdm(enumerate(val_loader), total=val_loader.__len__())
    cls_mious = torch.zeros(cfg.shape_classes, dtype=torch.float32).cuda(non_blocking=True)
    cls_nums = torch.zeros(cfg.shape_classes, dtype=torch.int32).cuda(non_blocking=True)
    ...
    for idx, data in pbar:
        ...
        if cfg.so3_mode == 'random_rot':
            for _ in range(cfg.num_rot):
                r = random_rot(data['pos'].size(0)).to(data['pos'].device)
                data['pos'] = new_data['pos'] @ r
                data['x'][:, :3] = r.transpose(1, 2) @ new_data['x'][:, :3]
                if data['x'].size(1) == 6:
                    data['x'][:, 3:] = r.transpose(1, 2) @ data['x'][:, 3:]
                logits = 0
                for v in range(num_votes+1):
                    set_random_seed(v)
                    if v > 0:
            ...
            if cfg.rot_group == 'tet':
                ...
                raise ValueError()
            r = r.to(data['pos'].device).unsqueeze(1)
            for i in range(r.size(0)):
                data['pos'] = new_data['pos'] @ r[i]
                data['x'][:, :3] = r[i].transpose(1, 2) @ new_data['x'][:, :3]
                if data['x'].size(1) == 6:
                    data['x'][:, 3:] = r[i].transpose(1, 2) @ data['x'][:, 3:]
                logits = 0
                for v in range(num_votes+1):
                    set_random_seed(v)
                    if v > 0:
        ...
        # TODO: current multiple poses are not supported!
        if logits.size(0) != target.size(0):
            target = target.unsqueeze(1).repeat(1, logits.size(0) // target.size(0)).flatten(0, 1)
            data['cls'] = data['cls'].unsqueeze(1).repeat(1, logits.size(0) // data['cls'].size(0)).flatten(0, 1)
